...
def parse_args(arg_input=None):
    ...
    parser.add_argument('--auth ', metavar='auth_file', dest='auth_file',
                        help='file for reading/storing user authentication \
                            tokens')
    parser.add_argument('--album', metavar='album_name', dest='album_name',
                        help='name of photo album to create (if it \
                            doesn\'t exist). Any uploaded photos will \
...
def getAlbums(session, appCreatedOnly=False):
    ...
    while True:
        ...
        albums = session.get(
            'https://photoslibrary.googleapis.com/v1/albums', params=params).json()
...
# No matches, create new album
    ...
    if "id" in resp:
        ...
        logging.error("Could not find or create photo album '/{0}/'. \
            Server Response: {1}".format(album_title, resp))
        return None
